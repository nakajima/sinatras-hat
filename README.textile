...
When a request comes in to @/posts.yaml@, the YAML proc will be called, and passed the @params[:post]@
string. The value of that will be assigned to a new @Post@ object's attributes hash, which is then saved.
...
You can register new response formats using the @formats@ option:
  ...
  mount Post |klass, model|
    klass.formats[:ruby] = proc { |result| [result].flatten.map(&:inspect) }
  end
...
* Access controls using :if option.
* More documentation (show update action)
